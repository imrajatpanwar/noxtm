...
const TeamInbox = () => {
  ...
  const selectAccount = async (account) => {
    ...
    await fetchEmails(account._id);
  };

  const fetchEmails = async (accountId, page = 1) => {
    setLoadingEmails(true);
    try {
      const res = await axios.get(`/api/email-accounts/team-inbox/${accountId}`, {
        params: {
          page,
          limit: emailsPerPage,
          folder: 'INBOX'
  ...
  const handleRefresh = () => {
    if (selectedAccount) {
      fetchEmails(selectedAccount._id, currentPage);
    }
  };
                  ...
                  <>
                    ...
                    <div className="pagination">
                      <button
                        ...
                        onClick={() => fetchEmails(selectedAccount._id, currentPage - 1)}
